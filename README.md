...
- **Average Duration by PM** and **Average Duration by Country** widgets: show Count/Avg/Max Duration (Validated → End Date) per PM or per Country, excluding POC Booked records (a booking date isn't the same kind of duration). Each has its own independent Stage filter (narrows just that table). Sortable (click a column header), searchable (text box above each table), and drill-down enabled -- clicking a row filters POC Detail below to just that PM or Country (also excluding POC Booked, to match the row's stats) and scrolls to it. Uses the same generalized drill-down mechanism as the duration-bucket boxes, so bucket, stage, PM, and Country drill-downs can combine. HTML dashboard only (no Excel equivalent).
